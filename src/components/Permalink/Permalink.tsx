...
export interface PermalinkProps {
   ...
   */
  replace?: (arg: { parameters: Record<string, any>; search: string }) => void;
}
...
class Permalink extends PureComponent<PermalinkProps, PermalinkState> {
  ...
  componentDidUpdate(prevProps: PermalinkProps, prevState: PermalinkState) {
  ...
  onMapMoved() {
    ...
    this.setState({
      ...
      z: +`${Math.round(Number(`${parseFloat(String(mapView.getZoom()))}e+2`))}e-2`,
    });
  }
...
(Permalink as any).defaultProps = defaultProps;
